...
/**
 * Work item parameters and mappings holding <code>#{...}</code> placeholders. Lives here rather than with the engine's
 * own tests because resolving a placeholder needs an expression language on the classpath.
 */
public class WorkItemTest {

    private static KogitoProcessRuntime createKogitoProcessRuntime(RuleFlowProcess process) {
        KieBase kbase = KnowledgeBaseFactory.newKnowledgeBase();
        ((InternalRuleBase) kbase).addProcess(process);
