...
public final class BpmnFeelVariables {
    ...
    /** The names a placeholder sees besides the variables. */
    ...
    public static Map<String, Object> forInterpolation(KogitoProcessContext context) {
        ...
        variables.putAll(suppliedVariables(context));
        variables.putAll(processVariables(context));
        putInstances(variables, context);
        return variables;
    }

    /**
     * What a <code>#{...}</code> placeholder can see, from the scope the engine resolved it in: the variables visible
     * where the placeholder appears, and the same four instance names.
     */
    public static Map<String, Object> forInterpolation(ExpressionScope scope) {
        Map<String, Object> variables = new HashMap<>(asFeelValues(scope.variables()));
        putInstances(variables, scope.context());
        return variables;
    }

    private static void putInstances(Map<String, Object> variables, KogitoProcessContext context) {
        KogitoProcessInstance processInstance = context == null ? null : context.getProcessInstance();
        variables.put("processInstance", processInstance(processInstance));
        variables.put("nodeInstance", nodeInstance(context == null ? null : context.getNodeInstance()));
        variables.put("processInstanceId", processInstance == null ? null : processInstance.getStringId());
        variables.put("parentProcessInstanceId", processInstance == null ? null : processInstance.getParentProcessInstanceId());
    }
