...
class CiSummary {
    ...
    static void appendMermaidGraph(StringBuilder md,
        ...
        // Short alphabetic aliases (a, b, …, z, aa, ab, …) instead of full path-based IDs
        // to minimise diagram text size — the label still shows the human-readable short name.
        ...
        for (var path : upstream) {
            appendMermaidNode(md, path, "([", "])", aliases);
        }
        ...
        for (var u : all) {
            for (var v : reduced.getOrDefault(u, Set.of())) {
                md.append("    ").append(aliases.get(u)).append(" --> ").append(aliases.get(v)).append("\n");
            }
        }
        ...
        var upstreamIds = upstream.stream()
            ...
            .map(aliases::get).collect(Collectors.joining(","));
        ...
        if (isPr) {
            if (!upstreamGa.isBlank()) {
                ...
                md.append("mvn -T 1C --batch-mode --no-transfer-progress -fae")
                  .append(" -DskipTests -DskipITs -Denforcer.skip=true -Dcheckstyle.skip=true -Dformatter.skip=true -Darchunit.skip=true")
